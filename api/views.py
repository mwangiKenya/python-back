...
def download_sms_excel(request):
    wb = Workbook()
    ws = wb.active
    ws.title = "SMS"
    ws.append([
    ...
    bills = Billings.objects.all().order_by(
        "grp",
        "parent",
        "name"
    )
    for b in bills:
        message = (
            f"Dear {b.name}, "
            f"Your water bill is KES {b.bill}. "
            f"Previous balance KES {b.b_cd}. "
            f"Current balance KES {b.bal}. "
            f"Thank you."
        )
        ws.append([
            b.id,
    ...
    response["Content-Disposition"] = (
        'attachment; filename="SMS_Billings.xlsx"'
    )
    wb.save(response)
    return response